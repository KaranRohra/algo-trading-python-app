...
class KiteConnect(object):
    ...
    def __init__(
        ...
        self.headers = self.get_required_headers(user_id, password, totp_value)
        self.user_id = user_id
        self.password = password
        self.totp_value = totp_value
    ...
    def invalidate_refresh_token(self, refresh_token):
        ...
        return self._delete(
            "api.token.invalidate",
            params={"api_key": self.api_key, "refresh_token": refresh_token},
        )
    ...
    def order_history(self, order_id):
        ...
        return self._format_response(
            self._get("order.info", url_args={"order_id": order_id})
        )
    ...
    def instruments(self, exchange=None):
        ...
        """
        if exchange:
            return self._parse_instruments(
                self._get("market.instruments", url_args={"exchange": exchange})
            )
        else:
